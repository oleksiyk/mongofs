    ...
    .spread(function(file, parent) {
        ...
        if(/^(w|w\+|a|a\+|wx|wx\+|ax|ax\+)$/.test(flags)){
            ...
            if(file && /^(a|a\+)$/.test(flags)){
                // TODO: set position to the end of file
...
MongoFs.prototype.write = function(fd, buffer, offset, length, position) {
    var self = this;
    ...
    if(!fd || !fd.file || !/^(w|w\+|wx|wx\+|a|a\+|ax|ax\+)$/.test(fd.flags)){
        return Promise.reject(new MongoFsError('EBADF', 'Invalid file descriptor'))
    }
    ...
    if(!Buffer.isBuffer(buffer)){
        return Promise.reject(new MongoFsError('EINVAL', 'buffer argument should be Buffer or String'))
    }

    if(position){
        return Promise.reject(new MongoFsError('EINVAL', 'Writing at position is not supported yet'))
    }
    ...
    return _write(buffer.slice(offset, length))
};
...
MongoFs.prototype.createWriteStream = function() {
};

MongoFs.prototype.writeFile = function() {
};
